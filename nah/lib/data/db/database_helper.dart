...
class DatabaseHelper {
  ...
  Future<void> _createDB(Database db, int version) async {
    ...
    await db.execute('''
      CREATE TABLE $bookmarkTableName (
        ...
        $titleField $textType,
        $languageField $textType,
        $hymnCollectionTitleField $textType
      )
     ''');
  ...
  Future<void> insertHc(HymnCollection hymnCol) async {
    final db = await database;
    await db.insert(
      hymnColTableName,
      hymnCol.toMap(),
  ...
  Future<List<HymnCollection>> getHcs() async {
    final db = await database;
  ...
  Future<void> deleteHc(HymnCollection hc) async {
    ...
    await db.transaction((txn) async {
      final bms = await txn.query(
        bookmarkTableName,
        where: "$hymnCollectionTitleField = ?",
        whereArgs: [hc.title.toLowerCase()],
      );
      print(bms.toString());
      ...
      final bmsRecheck = await txn.query(
        bookmarkTableName,
        where: "$hymnCollectionTitleField = ?",
        whereArgs: [hc.title.toLowerCase()],
      );
      print(bmsRecheck.toString());
  ...
  Future<void> insertBm(Bookmark bookmark) async {
    final db = await database;
    await db.insert(
      bookmarkTableName,
      bookmark.toMap(),
  ...
  Future<List<Bookmark>> getBm() async {
    final db = await database;
  ...
  Future<void> deleteBm(Bookmark bm) async {
    ...
    db.delete(
      bookmarkTableName,
      where: '$idField = ? AND $hymnCollectionTitleField = ?',
      whereArgs: [bm.id, bm.hcTitle.toLowerCase()],
    );
  }
