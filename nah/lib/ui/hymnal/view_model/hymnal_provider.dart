...
class HymnalProvider extends ChangeNotifier {
  ...
  final HymnalRepository _hymnalRepo;
  final HymnRepository _hymnRepo;

  HymnalProvider(this._hymnRepo, this._hymnalRepo);
  ...
  Future<void> loadHymnals() async {
    ...
    _errorMessage = null;
    notifyListeners();
    final hymnalFetchResult = await _hymnalRepo.getHymnals();
  ...
  Future<void> fetchAndCacheHymnsForAllHymnals(List<Hymnal> hymnalList) async {
    ...
    for (Hymnal hymnal in hymnalList) {
      final hymnFetchResult = await _hymnRepo.getHymns(
